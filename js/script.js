...
function setupWebSocket() {
  ...
  ws.onmessage = (event) => {
    ...
    if (data.type === 'location' && data.userId === userId && !isAdmin) {
      ...
      checkNewMessages();
      if (document.getElementById('messages-container').style.display === 'block') {
          fetchMessages(); // Refresh messages if messages-container is open
      }
...
async function addPin() {
  ...
  try{
    ...
    if (postResponse.ok) {
      const pin = await postResponse.json();
      if (ws.readyState === WebSocket.OPEN) {
      ...
      alert(`Failed to add alert: ${errorText}`);
    }
  } catch (error) {
        console.error('Error adding pin:', error);
        alert('Failed to add pin due to a network error.');
    }
}
...
async function updateProfile() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      //showMap();
      ws.send(JSON.stringify({ type: 'profileUpdate'}));
